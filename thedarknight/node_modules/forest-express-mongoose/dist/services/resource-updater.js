...
function ResourceUpdater(model, params, record) {
  ...
  this.perform = function () {
    return new P(function (resolve, reject) {
      recordId = record._id; // NOTICE: Old versions of MongoDB (2.X) seem to refuse the presence of
      //         the _id in the $set. So we remove it. It is useless anyway.

      delete record._id;
      var query = model.findByIdAndUpdate(recordId, {
        $set: record
      }, {
        new: true,
        runValidators: true
      ...
      query.lean().exec(function (err, recordUpdated) {
        if (err) {
          return reject(err);
        }

        return resolve(recordUpdated);
      });
      ...
      if (error.message.indexOf('Cast to') > -1 && error.message.indexOf('failed for value') > -1) {
        ...
        Interface.logger.error("Cannot update the ".concat(modelName, " #").concat(recordId, " because of an unexpected issue: ").concat(error));
      }
